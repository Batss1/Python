...
for item in vend_list:
    if item[0] not in cod_list:
        if item[2] == 999: 
            ...
            item[2] = 998  #altera o valor da situação caso o código não esteja na lista de produtos
            vend_canc.append(item)
...
diverg = { 
          ...
          998     : "Código de Produto não encontrado",
...
for erro in vend_canc:
    ...
    err_sit = erro[2]
    if err_sit == 998: 
        arq_diverg.write(f"Linha {err_linha} - {diverg[err_sit]} {erro[0]}\n")
    else:
        arq_diverg.write(f"Linha {err_linha} - {diverg[err_sit]}\n") 
...
def vendas_canal():
    ...
    for item in vend_list:
        if item[2] == 100 or item[2] == 102 or item[2] == 998:
            if item[3] == 1:
                soma_Repre += item[1]
            elif item[3] == 2:
                soma_web   += item[1]
...
print("Análise do Estoque Operacional concluido\n")